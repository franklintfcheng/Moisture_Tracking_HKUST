...
%         the rest are handles to lines that correspond to that axis lines.  
  ...
  %  get current axis
  cah = gca;
  linewidth = cah.LineWidth; %<--- add by me. To inherit linewidth from current axes.
  fontsize  = cah.FontSize;  %<--- add by me. To inherit fontsize  from current axes.
...
%  assume axes are added on right, then left, then right, etc.
  ...
  axcompleft = 0.06*1200/fig_width;    %<- adjust the interval of axes on LHS by figure width. (optimal)
  if numax == 1
    axcompright = 0.0;
  else 
    axcompright = 0.06*1200/fig_width; %<- adjust the interval of axes on RHS by figure width. (optimal)
  end
...
%  plot in new axis to get the automatically generated ylimits
  ...
  set(h_ax,'ylim',yl2);
  set(h_ax,'linewidth',linewidth); %<--- added by myself.
  set(h_ax,'fontsize',fontsize);   %<--- added by myself.
