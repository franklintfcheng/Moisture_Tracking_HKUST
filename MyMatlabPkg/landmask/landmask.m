...
%% My example of using this function. (Fandy)

% lon = [0:180];    res_lon = diff(lon(1:2));
% lat = [-45:45];   res_lat = diff(lat(1:2));
% 
...
% m_proj(map_proj,'longitudes',[min(lon)  max(lon)], 'latitudes', [min(lat)  max(lat)]);
% 
