...
%         'headstyle',1      1-4: arrow types.
%         'shaftwidth',1     points: width of arrow shaft
%         'centered', 'no'   arrow comes from specified x,y point
%                            'yes' to make x,y the arrow
%                                center instead of its tail
%                            'tail' makes the arrow point to the
%                                given x,y location.
...
%                             OUT of the axes.
%          'curvature',0  degrees: angle of a circular arc for the
%                             arrow body (to show curved arrows). Postive
%                             for rightward curvature.
%
...
% 23/May/23 - added 'shaftrotation' parameter for curved arrows.
...
if nargin==0  % demo
  ...
  %  orient tall
    ...
    m_grid('linestyle',':','xtick',[-135:5:-110],'linewi',2,'color',[.5 .5 .5],'fontsize',6);
    ...
    hpv2 = m_vec(100, [-119 -119 -119], [53 53 53], ...
    ...
    % can reasonably be.
    hpv3 = m_vec(100, vlon+12, vlat+2, uu, vv, 'm', ...
...
if istr0 > 0    % There are strings.
   ...
   while ~isempty(keyvars) && ~finished
      ...
      elseif strcmp(kv, 'headangle')
         headangle = value;
      elseif strcmp(kv, 'headstyle')
         headstyle = value;
      elseif strcmp(kv, 'shaftwidth')
         shaftwidth = value/72;
      elseif strcmp(kv, 'curvature')
         curvature = value;
      elseif strcmp(kv, 'centered')  && lower(value(1)) == 'y'
         centered = 1;
      elseif strcmp(kv, 'centered')  && lower(value(1)) == 't'
         centered = -1;
      elseif strcmp(kv, 'centered')  && lower(value(1)) == 'n'
         centered = 0;
...
if ~isempty(key) && (curvature==0), Ang = 0; end
...
nvert=7+2*ceil(abs(curvature)); % Add two vertices for every  degrees of rotation


nV = nvert*nvec;  % number of Vertices: 7 per arrow.
Vert = zeros(nvec,3); 
...
if curvature==0
    for k=1:nvec
      Vx(:,k)=Xzero(k)+[linspace(0,(L(k)-HL(k)*hval),2)  (L(k)-HL(k)) L(k) (L(k)-HL(k)) linspace((L(k)-HL(k)*hval),0,2)]';
      Vy(:,k) =[repmat(W(k)/2,1,2)           HW(k)/2   0 -HW(k)/2  repmat(-W(k)/2,1,2) ]';
      Vz(:,k) = repmat(Z(k),1,4+3)';
     end
    Vert=[Vx(:) Vy(:) Vz(:)];
else
    rad=L/(curvature*pi/180);
    for k=1:nvec
      Vth(:,k)=(Xzero(k)+[linspace(0,(L(k)-HL(k)*hval),2+abs(curvature))  (L(k)-HL(k)) L(k) (L(k)-HL(k)) linspace((L(k)-HL(k)*hval),0,2+abs(curvature))]')/rad(k);
      Vr(:,k) =[repmat(W(k)/2,1,2+abs(curvature))           HW(k)/2   0 -HW(k)/2  repmat(-W(k)/2,1,2+abs(curvature)) ]'+rad(k);
      Vz(:,k) = repmat(Z(k),1,4+abs(curvature)*2+3)';
     end
     Vert=[Vr(:).*sin(Vth(:)) Vr(:).*cos(Vth(:)) Vz(:)];  
     Vert(:,2)=Vert(:,2)-rad(ii);
end
...
Faces = [1:nV].';            %Top
Faces = reshape(Faces,nvert,nvec).';
...
if strcmp(edgeclip,'off')
 hp = patch('Faces', Faces, 'Vertices', Vert, 'tag', 'm_vec','clipping','off');
  ...
  [LG,LN]=m_xy2ll(reshape(Vert(:,1),nvert,nvec),reshape(Vert(:,2),nvert,nvec)); % Converts vertices in 7 point lines (i.e. columns) in lat/long
