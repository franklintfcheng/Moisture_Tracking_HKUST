%%
function ClimIndex = read_ClimIndex(varargin)
    ...
    if ismember(type, {'MJO_format'})
        ...
        ClimIndex.(indexname)(:, 1) = data_date_yyyymmdd;
        ClimIndex.(indexname)(:, 2) = data(:,7);    %/ Amplitude
        ClimIndex.(indexname)(:, 3) = data(:,6);    %/ Phase     phase == 999 means missing values!
        ...
        if ismember(type, {'Stndrd'})
            data = data(:,2:end);
            ClimIndex.(indexname) = nan(length(datayear)*12,1);
            for t = 1:length(datayear)
                ClimIndex.(indexname)(:, 1) = data_date_yyyymm;
                ClimIndex.(indexname)((1:12)+(t-1)*12, 2) = data(t,:);
            end

        elseif ismember(type, {'OneCol'})
            data = data(:,2);
            ClimIndex.(indexname)(:, 1) = data_date_yyyymm;
            ClimIndex.(indexname)(:, 2) = data;
        ...
        if mod(length(ClimIndex.(indexname)(:, 2)), 12) ~= 0
