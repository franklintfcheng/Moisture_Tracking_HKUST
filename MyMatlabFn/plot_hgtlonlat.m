function plot_hgtlonlat(varargin)
    ...
    pnames       = {'contf_data', 'contf_hori', 'contf_hgt', 'colmap', 'contf_levels', 'pcolor_mode', 'cbar_mode', 'cbar_location', 'cbar_interval', 'cbar_YTick', 'cbar_YTickLabel', 'draw_cbar_only',...
                    ...
                    'vec_hori_data','vec_hgt_data', 'vec_hori', 'vec_hgt', 'vec_hori_factor', 'vec_hgt_factor', 'qscale', 'vector_color', 'vector_linewidth', 'vector_step_hori', 'vector_step_hgt', 'draw_refvec_only', 'refvec_hori_mag', 'refvec_hgt_mag',...
                    ...
                    'fontsize',  'cbar_fontsize', 'linewidth', 'grid_mode', 'map_xlim', 'map_ylim', 'map_xticks', 'map_yticks', 'map_xticklabels', 'map_yticklabels', 'map_ylabel',...
...
%%
    ...
    %/
    %/ This function is designed for plotting the cross-section (height-lon
    %/ or height-lat) by overlaying contourf, contour, vector and line plots
    ...
    if isempty(cbar_fontsize)
        cbar_fontsize = fontsize*1.5;
    end
...
%         set(gcf, 'Position', get(0, 'Screensize'));
    ...
    if ~isempty(contf_data)
        ...
        if draw_cbar_only
            ...
            cb = colorbar;
            set(cb, 'YTick', cbar_YTick, 'YTickLabel', cbar_YTickLabel, 'Fontsize', cbar_fontsize,...
...
%                 set(cb, 'Position', pos);
        ...
        elseif draw_refvec_only == 0         
            if draw_refvec_only    %/ if draw_refvec_only, we dont plot contourf.
            ...
            if cbar_mode
                ...
                set(cb, 'YTick', cbar_YTick, 'YTickLabel', cbar_YTickLabel, 'Fontsize', cbar_fontsize, 'Location', cbar_location)   
                set(get(cb,'Title'),'String',contf_unit, 'Fontsize', cbar_fontsize)
...
%         if isempty(map_xticks)          map_xticks = contf_hori(mod(contf_hori, 5) == 0); end
        ...
        %/ Add degE degN etc to map_xticklabels
        if zm_or_mm == 1
            map_xticklabels_str = string(map_xticklabels);
            for i = 1:length(map_xticklabels)
                if map_xticklabels(i) == 0
                    str_deg = '';
                elseif map_xticklabels(i) < 0
                    str_deg = 'S';
                elseif map_xticklabels(i) > 0
                    str_deg = 'N';
                end
                map_xticklabels_str(i) = sprintf('%s%s%s',map_xticklabels_str(i),char(176),str_deg);
            end
...
%         end
        ...
        xticklabels(ax_ori,  map_xticklabels_str)
