function output = readpart10_testing(directory,i,numpart,varargin)
...
% output.itramem = transpose(dataAsInt(6,1:numpart)); %/ commented out (FC)

% output.vars = transpose(dataAsFloat(7:13,1:numpart));
output.vars = transpose(dataAsFloat([9,11,13],1:numpart)); %/ only extract the needed columns. (FC)
